...
import { FaArrowLeft, FaKey, FaWallet, FaPaperPlane, FaPlus, FaTrash, FaDownload, FaCheckCircle, FaTimesCircle, FaFileExport, FaFileImport } from 'react-icons/fa';
...
export default function MexcPage() {
  ...
  useEffect(() => {
    const savedApiKey = sessionStorage.getItem('mexc_api_key');
    const savedApiSecret = sessionStorage.getItem('mexc_api_secret');
      ...
      try {
        ...
        if (config.exchange === 'MEXC') {
          ...
          if (config.apiKey && config.apiSecret) {
            ...
            sessionStorage.setItem('mexc_api_key', config.apiKey);
            sessionStorage.setItem('mexc_api_secret', config.apiSecret);
            setIsAuthenticated(true);
          }
          // Restore withdrawals
  ...
  useEffect(() => {
    if (balances.length > 0 && coins.length > 0 && withdrawals.length === 0) {
      ...
      const firstNetwork = networks.length > 0
        ? (networks[0].netWork || networks[0].network)
        : '';
  ...
  useEffect(() => {
    if (coins.length > 0 && withdrawals.length > 0) {
      const updated = withdrawals.map(w => {
        ...
        // If current network doesn't match any netWork value, reset to first available
        const hasMatch = networks.some(n => n.netWork === w.network || n.network === w.network);
        if (!hasMatch && networks.length > 0) {
          return { ...w, network: networks[0].netWork || networks[0].network };
        }
        return w;
      });
  ...
  const handleConnect = async () => {
    if (!apiKey || !apiSecret) {
      setBalanceError('Please enter both API Key and API Secret');
      return;
    }
    sessionStorage.setItem('mexc_api_key', apiKey);
    sessionStorage.setItem('mexc_api_secret', apiSecret);
    setIsAuthenticated(true);
    await fetchBalance();
  };

  const handleDisconnect = () => {
    sessionStorage.removeItem('mexc_api_key');
    sessionStorage.removeItem('mexc_api_secret');
    setApiKey('');
    setApiSecret('');
  ...
  const updateWithdrawal = (index: number, field: keyof WithdrawalRequest, value: string) => {
    ...
    if (field === 'coin') {
      ...
      if (networks.length > 0) {
        updated[index].network = networks[0].netWork || networks[0].network;
      }
    }
  ...
  const handleWithdraw = async () => {
    ...
    setResults([]);
    try {
      const response = await fetch('/api/mexc/withdraw', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ apiKey, apiSecret, withdrawals }),
      });
  ...
  const exportResults = () => {
    ...
    a.href = url;
    a.download = `mexc-withdrawals-${Date.now()}.csv`;
    a.click();
  };
  ...
  const importConfig = () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (config.exchange !== 'MEXC') {
          alert('Invalid config file. This file is not for MEXC.');
          return;
        }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <SiMxlinux className="text-3xl text-blue-600" />
                <h1 className="text-2xl font-bold text-gray-900">MEXC Withdrawal</h1>
              </div>
            </div>
            {isAuthenticated && (
              <div className="flex items-center gap-2">
                <button onClick={importConfig} className="flex items-center gap-2 px-4 py-2 text-sm text-blue-600 hover:bg-blue-50 rounded-lg">
                  <FaFileImport /> Import
                </button>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="bg-white rounded-2xl p-8 shadow-lg">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-900">Connect to MEXC</h2>
              </div>
                      ...
                      <div>
                        ...
                        >
                          {getNetworksForCoin(withdrawal.coin).map(n => {
                            const optionValue = n.netWork || n.network;
                            return (
                              <option key={n.network} value={optionValue}>
